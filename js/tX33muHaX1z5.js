...
$(window).load(function(){
	...
	$(document).mousemove(function(e){
		...
		if(!menuPositionFixed)
		{
			...
			{
				var menuY = e.pageY;
				var maxY = $('#body-wrapper').height()-$('#menu-container').height()-120;
	...
	$('#bodyLoading').animate({opacity:'0', top:-200}, 300, 'easeOutBack', function(){
		$(this).remove();
	});
	$('#body-wrapper').delay(250).animate({opacity:'1'}, 1000);
...
function doSize(){
	...
	// Убрано для совместимости с мобилами
	// if(mobileDevice)
	// 	winW = minWidth;
...
function showMenu(){
	...
	$('#menu-container').stop(true).animate({left:0}, 400, 'easeOutExpo');

	$('#logo img').stop(true).animate({width:(logoData.width), height:(logoData.height), opacity:'1'}, 400, 'easeOutBack', function(){
		$('#menu-container ul.menu > li').each(function(i,el){
			$(el).find('> a > span').stop(true).delay(100*i).animate({top:0}, 200,'easeOutBack');

			if(!mobileDevice)
			{
				$(el).find('> a').hover(function(event){
					$(this).parent().find('ul').stop().show().animate({opacity:'1'}, 200, 'easeOutExpo');
					clearTimeout(subMenuTimer);
				...
				$(el).find('> a').click(function(event){
					$(this).parent().find('ul').stop().show().animate({opacity:'1'}, 400, 'easeOutExpo');
					closeSubMenu($(this).parent().attr('id'));
					menuItemOver(this);
				});
			}
